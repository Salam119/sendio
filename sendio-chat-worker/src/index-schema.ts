const CHAT_INDEX_SCHEMA = [
  ...
  `
    CREATE TABLE IF NOT EXISTS user_chat_settings (
      user_id TEXT PRIMARY KEY,
      profile_chat_enabled INTEGER NOT NULL DEFAULT 1
  ...
  `,
  `
    CREATE INDEX IF NOT EXISTS chat_user_moderation_status_idx
    ON chat_user_moderation(status, updated_at DESC)
  `,
  `
...
async function ensureColumn(
  ...
  if (result.results.some((column) => column.name === columnName)) {
    return;
  }

  await database
    .prepare(`ALTER TABLE ${tableName} ADD COLUMN ${definition}`)
    .run();
}

export function ensureChatIndexSchema(
  database: D1Database,
): Promise<void> {
  ...
  schemaPromise = (async () => {
    ...
    for (const statement of CHAT_INDEX_SCHEMA) {
