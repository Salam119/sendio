...
function parseArgs(argv) {
  ...
  if (!indexDir) {
    throw new Error('Use --index with the generated BCE index directory.');
  }
...
async function uploadAndVerify({ client, bucket, key, filePath, contentType, contentEncoding, expectedSha256 }) {
  ...
  await client.send(
    new PutObjectCommand({
      ...
      CacheControl: key.endsWith('/current.json')
        ? 'no-cache, max-age=0'
...
async function main() {
  ...
  await loadLocalEnv();
  const { endpoint, accessKeyId, secretAccessKey, bucket } = requireEnvironment();
  ...
  const versionDirectory = path.join(indexDir, `v${current.extract_number}`);
  ...
  if (!Array.isArray(manifest.shards) || manifest.shards.length === 0) {
    throw new Error('Invalid BCE manifest.');
  }
  ...
  for (let index = 0; index < manifest.shards.length; index += 1) {
    ...
    const filePath = path.join(versionDirectory, `${shard.prefix}.ndjson.gz`);
    const fileInfo = await stat(filePath);

    if (fileInfo.size !== shard.size_bytes) {
      throw new Error(`Local size mismatch for ${shard.prefix}.ndjson.gz.`);
    }

    process.stdout.write(
      `Uploading shard ${index + 1}/${manifest.shards.length}: ${shard.prefix}\n`,
    );
  ...
  await uploadAndVerify({
    client,
    bucket,
    key: 'bce/current.json',
    filePath: currentPath,
    contentType: 'application/json',
  });

  process.stdout.write(
    `BCE extract ${current.extract_number} uploaded and verified in R2.\n`,
  );
}
